...
$statut = $_GET['statut'] ?? '';
$page = $_GET['page'] ?? 1;
$limit = 20; // Nombre d'utilisateurs par page
$offset = ($page - 1) * $limit;
...
if ($statut === 'actif') {
    $whereConditions[] = "u.est_actif = 1";
} elseif ($statut === 'inactif') {
    $whereConditions[] = "u.est_actif = 0";
}
...
$countSql = "SELECT COUNT(*) as total FROM utilisateurs u $whereClause";
...
// Traitement des actions (suppression, activation/désactivation, changement de rôle)
if (isset($_POST['action']) && isset($_POST['user_id'])) {
    ...
    try {
        switch ($_POST['action']) {
            ...
            case 'toggle_active':
                $stmt = $pdo->prepare("SELECT est_actif FROM utilisateurs WHERE id = ?");
                $stmt->execute([$user_id]);
                $currentStatus = $stmt->fetch(PDO::FETCH_ASSOC)['est_actif'];
                
                $stmt = $pdo->prepare("UPDATE utilisateurs SET est_actif = ? WHERE id = ?");
                $stmt->execute([$currentStatus ? 0 : 1, $user_id]);
            ...
            case 'toggle_role':
                ...
                if ($user_id == $_SESSION['user_id'] && $newRole === 'client') {
        ...
        header("Location: gestion_utilisateurs.php?" . $_SERVER['QUERY_STRING']);
        exit;
...
<head>
    ...
    <title>Gestion des Utilisateurs - E-commerce</title>
            ...
            <div class="col-lg-10 main-content">
                ...
                <div class="welcome-card">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h1 class="display-6 fw-bold mb-2">Gestion des Utilisateurs</h1>
                            <p class="mb-0">Gérez les comptes utilisateurs de votre boutique en ligne</p>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="ajouter_utilisateur.php" class="btn btn-light btn-custom">
                ...
                <div class="row mb-4">
                    <div class="col-md-3">
                        <div class="stats-card bg-white">
                            <div class="stats-number text-primary"><?= $totalUtilisateurs ?></div>
                    ...
                    <div class="col-md-3">
                        <div class="stats-card bg-white">
                            <div class="stats-number text-success">
                                <?php 
                                $stmt = $pdo->query("SELECT COUNT(*) as count FROM utilisateurs WHERE est_actif = 1");
                    ...
                    <div class="col-md-3">
                        <div class="stats-card bg-white">
                            <div class="stats-number text-info">
                                <?php 
                                $stmt = $pdo->query("SELECT COUNT(*) as count FROM utilisateurs WHERE role = 'client'");
                                echo $stmt->fetch(PDO::FETCH_ASSOC)['count'];
                                ?>
                            </div>
                            <div>Clients</div>
                ...
                <div class="form-card">
                    ...
                    <div class="card-body">
                        <form method="GET" class="row g-3">
                            <div class="col-md-4">
                                <input type="text" class="form-control" name="search" placeholder="Nom, prénom ou email..." value="<?= htmlspecialchars($search) ?>">
                            </div>
                            <div class="col-md-3">
                ...
                <div class="table-card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><i class="fas fa-list me-2"></i>Liste des Utilisateurs (<?= $totalUtilisateurs ?>)</h5>
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($utilisateurs as $utilisateur): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge <?= $utilisateur['role'] === 'admin' ? 'badge-admin' : 'badge-client' ?>">
                                                        <?= $utilisateur['role'] === 'admin' ? 'Administrateur' : 'Client' ?>
                                                    </span>
                                                </td>
                                                <td>
                                                    <?php if (!$utilisateur['est_actif']): ?>
                                                ...
                                                <td>
                                                    <?php if ($utilisateur['derniere_connexion']): ?>
                                                        <small><?= date('d/m/Y H:i', strtotime($utilisateur['derniere_connexion'])) ?></small>
                                                    <?php else: ?>
                                                ...
                                                <td class="table-actions">
                                                    ...
                                                    <form method="POST" class="d-inline" onsubmit="return confirm('Êtes-vous sûr de vouloir changer le statut de cet utilisateur ?')">
                                                        <input type="hidden" name="user_id" value="<?= $utilisateur['id'] ?>">
                                                        <input type="hidden" name="action" value="toggle_active">
                                                        <button type="submit" class="btn btn-sm <?= $utilisateur['est_actif'] ? 'btn-outline-warning' : 'btn-outline-success' ?>" 
                                                                title="<?= $utilisateur['est_actif'] ? 'Désactiver' : 'Activer' ?>"
                                                                <?= $utilisateur['id'] == $_SESSION['user_id'] ? 'disabled' : '' ?>>
                                                            <i class="fas fa-power-off"></i>
                                                        </button>
                                                    </form>
                                                    
                                                    <form method="POST" class="d-inline" onsubmit="return confirm('Êtes-vous sûr de vouloir changer le rôle de cet utilisateur ?')">
                                                        <input type="hidden" name="user_id" value="<?= $utilisateur['id'] ?>">
                                                        <input type="hidden" name="action" value="toggle_role">
                                                        <button type="submit" class="btn btn-sm <?= $utilisateur['role'] === 'admin' ? 'btn-outline-secondary' : 'btn-outline-warning' ?>" 
                                                                title="<?= $utilisateur['role'] === 'admin' ? 'Rétrograder en client' : 'Promouvoir admin' ?>"
                                                                <?= $utilisateur['id'] == $_SESSION['user_id'] ? 'disabled' : '' ?>>
                                                            <i class="fas fa-user-shield"></i>
                                                        </button>
                                                    </form>
